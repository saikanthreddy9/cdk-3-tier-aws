...
with open("stacks/scripts/install_node.sh") as f:
    user_data = f.read()


class AlbBackendStack(core.Stack):
    def __init__(self,
        ...
        alb = elb.ApplicationLoadBalancer(
            self,
            f'{prefix_name}-private-alb',
            vpc=vpc,
            security_group=sg,
            internet_facing=False,
            load_balancer_name=f'{prefix_name}-private-alb')

        target_group = elb.ApplicationTargetGroup(
            self,
            f'{prefix_name}-private-alb-tg',
            port=80,
            vpc=vpc,
            target_type=elb.TargetType.INSTANCE,
            target_group_name=f'{prefix_name}-private-alb-tg',
            stickiness_cookie_duration=core.Duration.days(1))

        alb.add_listener(f'{prefix_name}-private-alb-listener',
                         port=80,
                         open=True,
        ...
        self.asg = autoscaling.AutoScalingGroup(
            self,
            f'{prefix_name}-backend-asg',
            auto_scaling_group_name=f'{prefix_name}-backend-asg',
        ...
        core.Tags.of(self.asg).add("Name", f'{prefix_name}-private-asg-instance')
